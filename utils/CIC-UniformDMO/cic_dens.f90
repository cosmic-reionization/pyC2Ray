subroutine cic_dens(fbase,ithread,nthread,nfiles)
  ...
  real,allocatable::pos(:,:),vel(:,:),mass(:), ids(:)
  ...
  do j=0,(nfiles/nthread)-1
     ...
     allocate(pos(3,npart_in_file),vel(3,npart_in_file),ids(npart_in_file), mass(npart_in_file))
...
!     write(*,*) header,'H',n_o_b
     ...
     !write(*,*) 'ids', minval(ids), maxval(ids)
     
     READ(iunit) mass
     !write(*,*) 'mass', minval(mass), maxval(mass)
...
!!$        
        ...
        count_loc_grid(ix,iy,iz)=count_loc_grid(ix,iy,iz)+1
        !write(*,*) 'mass = ', dble(mass(i))
        ...
        cic_loc_grid(ix,iy,iz)=cic_loc_grid(ix,iy,iz)+nx0y0z0

        cic_loc_grid(ix1,iy,iz)=cic_loc_grid(ix1,iy,iz)+nxsy0z0
        cic_loc_grid(ix,iy1,iz)=cic_loc_grid(ix,iy1,iz)+nx0ysz0
        cic_loc_grid(ix,iy,iz1)=cic_loc_grid(ix,iy,iz1)+nx0y0zs
        cic_loc_grid(ix1,iy1,iz)=cic_loc_grid(ix1,iy1,iz)+nxsysz0
        cic_loc_grid(ix1,iy,iz1)=cic_loc_grid(ix1,iy,iz1)+nxsy0zs
        cic_loc_grid(ix,iy1,iz1)=cic_loc_grid(ix,iy1,iz1)+nx0yszs
        cic_loc_grid(ix1,iy1,iz1)=cic_loc_grid(ix1,iy1,iz1)+nxsyszs
