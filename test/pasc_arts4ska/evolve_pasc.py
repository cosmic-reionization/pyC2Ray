...
import pyc2ray
from pyc2ray.utils import printlog, display_time
from pyc2ray.utils.sourceutils import format_sources
from pyc2ray.load_extensions import load_c2ray, load_asora
...
def evolve3D_pasc(path_out, redshift, dt, dr,
             src_flux, src_pos,
    ...
    while not converged:
        ...
        if(rank == 0):            
            ...
            printlog('---> xh_av %s %.3f %d' %(path_out, redshift, niter), logfile, quiet)
            np.save(file='%sxfrac_z%.3f_%d.npy' %(path_out, redshift, niter), arr=xh_intermed)
            np.save(file='%sIonRates_z%.3f_%d.npy' %(path_out, redshift, niter), arr=phi_ion)
    ...
    if(rank == 0):
        ...
        printlog("Multiple source convergence reached after %d ray-tracing iterations." %niter, logfile, quiet)
        xh_new = xh_intermed
