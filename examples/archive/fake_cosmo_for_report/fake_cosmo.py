...
if args.daint:
    paramfile = "parameters_daint.yml"
else:
...
print(fact)
if numsrc is None:
    srcpos_raw = sources_list[:, :3]
else:
    srcpos_raw = sources_list[:numsrc, :3]
print(srcpos_raw)
if numsrc is None:
    normflux_raw = fact * sources_list[:, 3] / 1e48
else:
    normflux_raw = fact * sources_list[:numsrc, 3] / 1e48
...
pc2r.printlog(
    f"Raytracing radius: {r_RT:n} grid cells (= {sim.dr_c * u.cm.to('Mpc'):.3f} comoving Mpc)",
    sim.logfile,
)
for k in range(len(zred_array) - 1):
    ...
    for t in range(num_steps_between_slices):
        ...
        pc2r.printlog(
            f"Mean density is: {sim.ndens.mean():.3e}, mean ionization fraction: {sim.xh.mean():.3e}",
            sim.logfile,
        )
