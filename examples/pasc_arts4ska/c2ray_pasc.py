...
import pyc2ray
from pyc2ray.c2ray_base import YEAR, C2Ray, m_p, msun2g
...
class C2Ray_PASC(C2Ray):
    ...
    def evolve3D(self, path_out, redshift, dt, src_flux, src_pos):
        ...
        if NumSrc >= self.nprocs and self.mpi:
            self.xh, self.phi_ion, self.coldens = evolve3D_pasc(
                path_out=path_out,
                redshift=redshift,
                dt=dt,
                dr=self.dr,
    ...
    def _sources_init(self):
        ...
        if self.fesc_kind == "constant":
            self.printlog(
                f"Using constant escaping fraction model with f0_esc = %.1f"
