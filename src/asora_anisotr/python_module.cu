...
static PyObject *asora_do_all_sources(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "dOddOOOiiddi", &R, &coldensh_out, &sig, &dr,
                        &ndens, &xh_av, &phi_ion, &NumSrc, &m1, &minlogtau,
                        &dlogtau, &NumTau))
    return NULL;
  ...
  do_all_sources_gpu(R, coldensh_out_data, sig, dr, ndens_data, xh_av_data,
                     phi_ion_data, NumSrc, m1, minlogtau, dlogtau, NumTau);
...
static PyObject *asora_source_data_to_device(PyObject *self, PyObject *args) {
  ...
  double *rdir_data = (double *)PyArray_DATA(rdir);
  double *angl_data = (double *)PyArray_DATA(angl);

  source_data_to_device(pos_data, flux_data, rdir_data, angl_data, NumSrc);
...
static struct PyModuleDef asoramodule = {
    PyModuleDef_HEAD_INIT, "libasora_anisotr", /* name of module */
