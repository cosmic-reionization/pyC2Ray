...
// ========================================================================
void device_init(const int &N, const int &num_src_par, const int &mpi_rank,
                 const int &num_gpus) {
  // int dev_id = 0;
...
void photo_table_to_device(double *thin_table, double *thick_table,
                           const int &NumTau) {
  // Copy thin table
  cudaMalloc(&photo_thin_table_dev, NumTau * sizeof(double));
  cudaMemcpy(photo_thin_table_dev, thin_table, NumTau * sizeof(double),
             cudaMemcpyHostToDevice);
  // Copy thick table
  cudaMalloc(&photo_thick_table_dev, NumTau * sizeof(double));
  cudaMemcpy(photo_thick_table_dev, thick_table, NumTau * sizeof(double),
             cudaMemcpyHostToDevice);
}
void source_data_to_device(int *pos, double *flux, double *rdir, double *angl,
                           const int &NumSrc) {
  ...
  cudaMemcpy(src_flux_dev, flux, NumSrc * sizeof(double),
             cudaMemcpyHostToDevice);
  cudaMemcpy(rad_dir_dev, rdir, 3 * NumSrc * sizeof(double),
             cudaMemcpyHostToDevice);
