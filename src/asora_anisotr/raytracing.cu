...
void do_all_sources_gpu(
    ...
    {   
        ...
        {   
            ...
            evolve0D_gpu<<<gs,bs>>>(R,max_q,ns,NumSrc,NUM_SRC_PAR,src_pos_dev,src_flux_dev,rad_dir_dev,angles_dev,cdh_dev,sig,dr,n_dev,x_dev,phi_dev,m1,photo_thin_table_dev,photo_thick_table_dev,minlogtau,dlogtau,NumTau,last_l,last_r);
            ...
            for (int ipass = 0 ; ipass < Npass ; ipass++)
            {
                ...
                if (s < s_end)
                {
                    ...
                    if ((i >= last_l) && (i <= last_r) && (j >= last_l) && (j <= last_r) && (k >= last_l) && (k <= last_r))
                    {
                        ...
                        // Calculate dot product and cosine of the angle (here ijk are coord. in reference frame of the source, i0, j0 and k0)
                        double cos_thet = (i*xp + j*yp + k*zp)/sqrtf(i*i + j*j + k*k);

                        // Center to source (the shift is done to have the ijk in the reference frame of the box)
                        i += i0;
                        j += j0;
                        k += k0;
