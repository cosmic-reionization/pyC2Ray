...
#define MAX_COLDENSH                                                           \
  2e30 // Column density limit (rates are set to zero above this)
...
inline __device__ int mem_offst_gpu(const int &i, const int &j, const int &k,
                                    const int &N) {
  return N * N * i + N * j + k;
}
...
__device__ inline int cart2cache(const int &i, const int &j, const int &k,
                                 const int &N) {
...
// ========================================================================
void do_all_sources_gpu(const double &R, double *coldensh_out,
                        const double &sig, const double &dr, double *ndens,
                        double *xh_av, double *phi_ion, const int &NumSrc,
                        const int &m1, const double &minlogtau,
  ...
  // radius. Currently, this is set s.t. the radius equals the distance from the
  // source to the middle of the faces of the octahedron. To raytrace the whole
  // box, the octahedron bust be 1.5*N in size
  ...
  for (int ns = 0; ns < NumSrc; ns += NUM_SRC_PAR) {
    ...
    evolve0D_gpu<<<gs, bs>>>(R, max_q, ns, NumSrc, NUM_SRC_PAR, src_pos_dev,
                             src_flux_dev, rad_dir_dev, angles_dev, cdh_dev,
                             sig, dr, n_dev, x_dev, phi_dev, m1,
                             photo_thin_table_dev, photo_thick_table_dev,
  ...
  cudaError_t error =
      cudaMemcpy(phi_ion, phi_dev, meshsize, cudaMemcpyDeviceToHost);
  // cudaError_t error2 =
...
// ========================================================================
__global__ void
evolve0D_gpu(const double Rmax_LLS,
  ...
  /* The raytracing kernel proceeds as follows:
     ...
     requiring multiple iterations if the block size is smaller than the number
     of cells in the shell (loop "B")
  ...
  */
  ...
  if (ns < NumSrc) {
    ...
    for (int q = 0; q <= q_max; q++) {
      ...
      // The threads have 1D indices 0,...,blocksize-1. We map these 1D indices
      // to the 3D positions of the cells inside the shell via the mapping
      // described in the paper. Since in general there are more cells than
      // threads, there is an additional loop here (B) so that all cells are
      // treated.
      int s_end;
      if (q == 0) {
        s_end = 1;
      ...
      for (int ipass = 0; ipass < Npass; ipass++) {
        // "s" is the index in the 1D-range [0,...,4q^2 + 1] that gets mapped to
        // the cells in the shell
        ...
        if (s < s_end) {
          ...
          // mapping is slightly different due to the part that is on the same
          // z-plane as the source)
          if (s < s_end_top) {
            sgn = 1;
          ...
          if ((i >= last_l) && (i <= last_r) && (j >= last_l) &&
            ...
            // Get radiation direction properties
            double xp = rad_dir[3 * ns + 0];
            double yp = rad_dir[3 * ns + 1];
            double zp = rad_dir[3 * ns + 2];
            double cos_thet_max = angles[ns];
...
#endif
            {
              ...
              if ((coldensh_in <= MAX_COLDENSH) &&
                ...
                if ((i == i0 && j == j0 && k == k0) ||
                  ...
                  double phi = photoion_rates_gpu(strength, coldensh_in, cdho,
                                                  vol_ph, sig, photo_thin_table,
                                                  photo_thick_table, minlogtau,
                                                  dlogtau, NumTau);
...
// ========================================================================
__device__ void cinterp_gpu(const int i, const int j, const int k, const int i0,
                            const int j0, const int k0, double &cdensi,
                            double &path, double *coldensh_out,
...
// ========================================================================
// Modified version of cinterp using a different coordinate system, for future
development
// ========================================================================
    ...
    const double sigma_HI_at_ion_freq)
{
    ...
    // Transform coordinates: origin at source location (i0,j0,k0) "s"
coordinate system
    ...
    // Transform coordinates: origin at lower left corner of cache array.
"tilde" or "t" coordinate system
    // range [0,2*qmax]
    it = is + qmax;
    jt = js + qmax;
    kt = ks + qmax;
...
// WIP: compute rates in a separate step ? This would require having a path and
coldensh_in grid (replace cdh_out by path)
...
)
{
    ...
    // 2. It must be within the maximum box size arount the source (see last_l
and last_r def above) <- This is also important to avoid race conditions at the
border
    // TODO: in the future, it may be an interesting optimization to limit the
number of threads launched in the first place,
    // rather than doing this "brute force" approach where about half of the
threads don't pass this "if" check and immediately return if (abs(i) + abs(j) <=
mq && (i >= last_l) && (i <= last_r) && (j >= last_l) && (j <= last_r) && (k >=
last_l) && (k <= last_r))
    {
        // Get source properties
        int i0 = src_pos[3*ns + 0];
        ...
        int pos[3];
        double path;
        double coldensh_in;                                // Column density to
the cell double nHI_p;                                      // Local density of
neutral hydrogen in the cell double xh_av_p; // Local ionization fraction of
cell
        ...
        #endif
        {
            ...
            // Get local ionization fraction & Hydrogen density
            xh_av_p = xh_av[mem_offst_gpu(pos[0],pos[1],pos[2],m1)];
            nHI_p = ndens[mem_offst_gpu(pos[0],pos[1],pos[2],m1)] * (1.0 -
xh_av_p);
...
density else
                ...
                coldensh_out[mem_offst_gpu(pos[0],pos[1],pos[2],m1)] =
coldensh_in + nHI_p * path;

                // Compute photoionization rates from column density. WARNING:
for now this is limited to the grey-opacity test case source if (coldensh_in <=
MAX_COLDENSH)
                {
                    #if defined(GREY_NOTABLES)
                    double phi =
photoion_rates_test_gpu(strength,coldensh_in,coldensh_out[mem_offst_gpu(pos[0],pos[1],pos[2],m1)],vol_ph,sig);
                    #else
                    double phi =
photoion_rates_gpu(strength,coldensh_in,coldensh_out[mem_offst_gpu(pos[0],pos[1],pos[2],m1)],vol_ph,sig,photo_table,photo_table,minlogtau,dlogtau,NumTau);
                    #endif
                    // Divide the photo-ionization rates by the appropriate
neutral density
