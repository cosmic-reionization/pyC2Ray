module chemistry
    ...
    subroutine do_chemistry(dt, ndens_p, temperature_start, & 
        ...
        real(kind=real64), intent(in) :: coldhi_p, coldhei_p, coldheii_p      ! column density
        ...
        do
            ...
            ! call thermal(dt, temperature_start%current, temperature_end%current, temperature_end%average, de, ndens_p, ion,phi)    
    ...
    real(kind=real64), intent(in) :: NHI, NHeI, NHeII
    real(kind=real64), intent(in) :: xHII_old, xHeII_old, xHeIII_old
    real(kind=real64), intent(in) :: dt, temp_p, n_e
    real(kind=real64), intent(in) :: phi_HI, phi_HeI, phi_HeII
    real(kind=real64), intent(in) :: heat_HI, heat_HeI, heat_HeII
    real(kind=real64), intent(in) :: clumping
    real(kind=real64), intent(out) :: xHII_av, xHeII_av, xHeIII_av
