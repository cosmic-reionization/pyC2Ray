...
evolve0D_gpu(const double Rmax_LLS,
             ...
             int *src_pos, double *src_flux, double *coldensh_out,
             const double sig, const double dr, const double *ndens,
             const double *xh_av, double *phi_ion, const int m1,
             const double *photo_thin_table, const double *photo_thick_table,
             const double minlogtau, const double dlogtau, const int NumTau,
             const int last_l, const int last_r) {
...
// When not in periodic mode, only treat cell if its in the grid
...
#endif
            {
              ...
              if ((coldensh_in <= MAX_COLDENSH) &&
                ...
                double phi = photoion_rates_test_gpu(
                    strength, coldensh_in,
