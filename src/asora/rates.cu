...
// ========================================================================
__device__ double
photoion_rates_gpu(const double &strength, const double &coldens_in,
  ...
  double phi_photo_in = prefact * photo_lookuptable(photo_thick_table, tau_in,
                                                    minlogtau, dlogtau, NumTau);

  if (abs(tau_out - tau_in) > TAU_PHOTO_LIMIT) {
    double phi_photo_out =
        prefact * photo_lookuptable(photo_thick_table, tau_out, minlogtau,
                                    dlogtau, NumTau);
    return phi_photo_in - phi_photo_out;
  } else {
    return prefact * (tau_out - tau_in) *
           photo_lookuptable(photo_thin_table, tau_out, minlogtau, dlogtau,
                             NumTau);
  }
...
// ========================================================================
__device__ double photo_lookuptable(const double *table, const double &tau,
