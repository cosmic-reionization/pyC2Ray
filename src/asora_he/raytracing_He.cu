...
void do_all_sources_gpu(
  ...
  auto error6 = cudaMemcpy(phi_heat_HeII, heat_HeII_dev, meshsize,
  ...
  auto error9 = cudaMemcpy(coldensh_out_heii, cdheii_dev, meshsize,
                           cudaMemcpyDeviceToHost);
}
...
__global__ void evolve0D_gpu(
    ...
    const double *heat_thick_table, const double minlogtau,
    const double dlogtau, const int NumTau, const int NumBin1,
      ...
      for (int ipass = 0; ipass < Npass; ipass++) {
        ...
        if (s < s_end) {
          ...
          if ((i >= last_l) && (i <= last_r) && (j >= last_l) &&
            ...
            double
                xheii_av_p; // Local helium second ionization fraction of cell
...
// When not in periodic mode, only treat cell if its in the grid
...
#endif
            {
              ...
              nHI_p = ndens[mem_offst_gpu(pos[0], pos[1], pos[2], m1)] *
                      (1.0 - abu_he_mass) * (1.0 - xh_av_p);

              // Get local HeI number density
              nHeI_p = ndens[mem_offst_gpu(pos[0], pos[1], pos[2], m1)] *
                       abu_he_mass * (1.0 - xhei_av_p - xheii_av_p);
              ...
              // array ATOMICALLY
              atomicAdd(coldensh_out_hi +
                            mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                        cdo_hi);
              atomicAdd(coldensh_out_hei +
                            mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                        cdo_hei);
              atomicAdd(coldensh_out_heii +
                            mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                        cdo_heii);
              ...
              if ((coldens_in_hi <= MAX_COLDENSH) &&
                ...
                for (int nf = 0; nf < NumFreq; nf += 1) {
                  ...
                  atomicAdd(phi_ion_HI +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            phi_HI);
                  atomicAdd(phi_ion_HeI +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            phi_HeI);
                  atomicAdd(phi_ion_HeII +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            phi_HeII);
                  atomicAdd(phi_heat_HI +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            heat_HI);
                  atomicAdd(phi_heat_HeI +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            heat_HeI);
                  atomicAdd(phi_heat_HeII +
                                mem_offst_gpu(pos[0], pos[1], pos[2], m1),
                            heat_HeII);
...
// ========================================================================
__device__ void
cinterp_gpu(const int i, const int j, const int k, const int i0, const int j0,
