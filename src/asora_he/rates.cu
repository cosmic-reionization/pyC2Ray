...
// ========================================================================
#define TAU_PHOTO_LIMIT                                                        \
  1.0e-7 // Limit to consider a cell "optically thin/thick"
#define S_STAR_REF                                                             \
...
__device__ double photoion_rates_gpu(
    const double &strength, const double &tau_in, const double &tau_out,
    const int &nf, const double &Vfact, const double *photo_thin_table,
    const double *photo_thick_table, const double &minlogtau,
    const double &dlogtau, const int &NumTau, const int &NumFreq) {
  ...
  double phi_photo_in =
      prefact * photo_lookuptable(photo_thick_table, nf, tau_in, minlogtau,
                                  dlogtau, NumTau);

  // printf("%i %e\n", nf, photo_thick_table[0+nf*NumTau]);
  if (abs(tau_out - tau_in) > TAU_PHOTO_LIMIT) {
    double phi_photo_out =
        prefact * photo_lookuptable(photo_thick_table, nf, tau_out, minlogtau,
                                    dlogtau, NumTau);
    return phi_photo_in - phi_photo_out;
  } else {
    return prefact * (tau_out - tau_in) *
           photo_lookuptable(photo_thin_table, nf, tau_out, minlogtau, dlogtau,
                             NumTau);
  }
...
// -DGREY_NOTABLES flag
// ========================================================================
__device__ double photoion_rates_test_gpu(const double &strength,
                                          const double &coldens_in,
                                          const double &coldens_out,
                                          const double &Vfact,
                                          const double &sig) {
...
// ========================================================================
__device__ double photo_lookuptable(const double *table, const int &nf,
                                    const double &tau, const double &minlogtau,
  ...
  if (log10(tau) < minlogtau) {
    ...
    // + 1.0;
    real_i =
        float(NumTau) * (log10(tau) - minlogtau) / (float(NumTau) * dlogtau) +
        1.0;
    // Find table index and weight for linear interpolation
    i0 = int(floor(real_i));
    i1 = int(ceil(real_i));
