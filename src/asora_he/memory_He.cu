...
void device_init(const int &N, const int &num_src_par, const int &num_freq) {
  ...
  if (error != cudaSuccess) {
    ...
    std::cout << ", with source batch size " << NUM_SRC_PAR << " and "
              << NUM_FREQ << " frequency bins." << std::endl;
  }
}
...
void tables_to_device(double *photo_thin_table, double *photo_thick_table,
                      double *heat_thin_table, double *heat_thick_table,
  ...
  cudaMemcpy(photo_thin_table_dev, photo_thin_table,
             int(NumTau * NumFreq) * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(heat_thin_table_dev, heat_thin_table,
             int(NumTau * NumFreq) * sizeof(double), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(photo_thick_table_dev, photo_thick_table,
             int(NumTau * NumFreq) * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(heat_thick_table_dev, photo_thick_table,
             int(NumTau * NumFreq) * sizeof(double), cudaMemcpyHostToDevice);
}
...
void source_data_to_device(int *pos, double *flux, const int &NumSrc) {
  ...
  cudaMemcpy(src_pos_dev, pos, 3 * NumSrc * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(src_flux_dev, flux, NumSrc * sizeof(double),
             cudaMemcpyHostToDevice);
}
