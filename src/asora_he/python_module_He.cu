...
static PyObject *asora_do_all_sources(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(
          args, "dOOOOOOiiidOOOOOOOOOOiiddi", &R, &coldensh_out,
          &coldenshei_out, &coldensheii_out, &sig_hi, &sig_hei, &sig_heii,
          &nbin1, &nbin2, &nbin3, &dr, &ndens, &xHI_av, &xHeI_av, &xHeII_av,
          &phi_ion_HI, &phi_ion_HeI, &phi_ion_HeII, &phi_heat_HI, &phi_heat_HeI,
          &phi_heat_HeII, &NumSrc, &m1, &minlogtau, &dlogtau, &NumTau))
    return NULL;

  // Error checking
  if (!PyArray_Check(coldensh_out) ||
      PyArray_TYPE(coldensh_out) != NPY_DOUBLE) {
    PyErr_SetString(PyExc_TypeError,
                    "coldensh_out must be Array of type double");
    return NULL;
  } else if (!PyArray_Check(coldenshei_out) ||
             PyArray_TYPE(coldenshei_out) != NPY_DOUBLE) {
    PyErr_SetString(PyExc_TypeError,
                    "coldenshei_out must be Array of type double");
    return NULL;
  } else if (!PyArray_Check(coldensheii_out) ||
             PyArray_TYPE(coldensheii_out) != NPY_DOUBLE) {
    PyErr_SetString(PyExc_TypeError,
                    "coldensheii_out must be Array of type double");
    return NULL;
  }
...
static PyObject *asora_tables_to_device(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOOii", &photo_thin_table, &photo_thick_table,
                        &heat_thin_table, &heat_thick_table, &NumTau, &NumFreq))
    return NULL;
  ...
  tables_to_device(photo_thin_table_data, photo_thick_table_data,
                   heat_thin_table_data, heat_thick_table_data, NumTau,
                   NumFreq);
...
static PyMethodDef asoraMethods[] = {
    {"do_all_sources", asora_do_all_sources, METH_VARARGS,
     "Do OCTA raytracing (GPU)"},
