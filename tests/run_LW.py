...
for k in range(len(zred_fine) - 1):
    ...
    if abs(current_z_coarse - zi) < 1e-4:
        ...
        if sim.dens_ND is not None:
            ...
            # First coarse update but not first redshift - read previous
            sim.read_density(z=zred_fine[k-1])
            sim.dens_ND_prev = sim.dens_ND.copy()
        else:
            # Very first redshift
        ...
        srcpos_massive, normflux_massive = sim.read_sources(
        ...
        # Massive sources remain the same as from last coarse step (srcpos_massive, normflux_massive)
    ...
    srclumK = sim.get_srclumK(
        k, 
        srcpos_massive, normflux_massive,  # Massive sources
    ...
    # ===================================================================
    if srcpos_massive is not None and srcpos_massive.size > 0:
        if srcpos_mh.size > 0:
            srcpos = np.concatenate([srcpos_massive, srcpos_mh], axis=1)
            normflux = np.concatenate([normflux_massive, normflux_mh])
        else:
            srcpos = srcpos_massive
            normflux = normflux_massive
    else:
        srcpos = srcpos_mh
    ...
    while sim_time < end_time:
        ...
        actual_dt = min(actual_dt, end_time - sim_time)
        ...
        if hasattr(sim, 'redshift_evol') and hasattr(sim, 'cosmo_evol'):
            sim.redshift_evol(sim_time + 0.5 * actual_dt)
            sim.cosmo_evol()
        elif hasattr(sim, 'cosmo_evolve'):
            sim.cosmo_evolve(actual_dt / 2.0)
    ...
    # Scale to the current redshift at end of interval
    if hasattr(sim, 'redshift_evol') and hasattr(sim, 'cosmo_evol'):
        sim.redshift_evol(sim_time)
        sim.cosmo_evol()
    elif hasattr(sim, 'cosmo_evolve_to_now'):
