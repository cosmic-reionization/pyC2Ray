...
class C2Ray_Thesan(C2Ray):
    ...
    def ionizing_flux(self, file, z, dt, rad_feedback=False, save_Mstar=False):
        ...
        if(std_opt == 0):
            ...
            idx_mass = np.digitize(x=np.log10(srcmass_msun), bins=self.mass_bins[i_tab]) - 1

            # loop trough the 
            for i_um in np.unique(idx_mass):
                mask_fit = idx_mass == i_um
                if (i_um != mass_mid.size):
                    if(self.pdf_data[i_tab, i_um].sum() != 0.):
                        prob = np.nan_to_num(self.pdf_data[i_tab, i_um]/self.pdf_data[i_tab, i_um].sum())
                        dotN_pyc2ray[mask_fit] = 10**(np.random.choice(a=dotN_mid, size=mask_fit.sum(), p=prob) + np.random.normal(loc=0, scale=0.1, size=mask_fit.sum()))
                    else:
                        ...
                        dotN_pyc2ray[mask_fit] = 10**(func(np.log10(srcmass_msun[mask_fit]), *popt) + np.random.normal(loc=0., scale=0.1, size=mask_fit.sum()))

            dotN_pyc2ray[~mask_ext] = 10**(func(np.log10(srcmass_msun[~mask_ext]), *popt) + np.random.normal(loc=0., scale=0.1, size=(1-mask_ext).sum()))
        else:
            dotN_pyc2ray = 10**(func(np.log10(srcmass_msun), *popt) + np.random.normal(loc=0, scale=std_opt, size=srcmass_msun.size))
        
        # TODO: this is for testing
        #self.dotN = dotN_pyc2ray

        # sum together masses into a mesh grid and get a list of the source positon and mass
        srcpos, dotN = bin_sources(srcpos_mpc=srcpos_mpc, mstar_msun=dotN_pyc2ray, boxsize=self.boxsize/self.cosmology.h, meshsize=self.N+1)
        ...
        self.printlog(' min, max halo (grid) mass : %.3e  %.3e [Msun] and min, mean, max number of ionising sources : %.3e  %.3e  %.3e [1/s]' %(srcmass_msun.min(), srcmass_msun.max(), normflux.min()*S_star_ref, normflux.mean()*S_star_ref, normflux.max()*S_star_ref))
    ...
    def _sources_init(self):
        ...
        """
        self.printlog(f" --- You are using the Thesan source model so:")
        self.printlog(f" NO stallar-to-halo relaction model.")
        self.printlog(f" NO stellar accretion model.")
        self.printlog(f" NO bustiness model for the star formation history.")
        self.printlog(f" NO escaping fraction model.")
        self.printlog(f" Instead reading fit tables created from Thesan simulations.")
