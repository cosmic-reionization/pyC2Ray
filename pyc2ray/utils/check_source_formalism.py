...
def classical_c2ray_model(g_gamma, Mh=None, z=None):
    if Mh is None: Mh = 10**np.linspace(9,13,20)*u.Msun
    if not isinstance(Mh,u.quantity.Quantity): Mh = Mh*u.Msun
    if z is None: z = np.linspace(5,20,30)
    mp = const.m_p
...
def exp_model(Nion, Mh=None, z=None, al_h=0.79,
            f0=0.1, Mt=1e8, Mp=1e10,
            g1=-0.3, g2=-0.3, g3=0, g4=0,
    ...
    mp = const.m_p
    if isinstance(z,float): z = np.array([z])
    fstar = stellar_to_halo_fraction(Mh.value, f0=f0, Mt=Mt, Mp=Mp,
...
out_exp = exp_model(2000, g1=0, g2=0) #made redshift independent to only see the impact of exp mass growth

fig, axs = plt.subplots(1,2,figsize=(13,5))
Mh_plot = [1e10,1e11,1e12]
lstyles = ['--', '-.', ':']
for ii,Mi in enumerate(Mh_plot):
    axs[0].semilogy(out_old['z'], out_old['Ndot'][:,np.abs(out_old['Mh'].value-Mi).argmin()],
        c=f'C{ii}', label=f'$M_\mathrm{{h}}=10^{{{np.log10(Mi):.0f}}} M_\odot$')
    axs[0].semilogy(out_exp['z'], out_exp['Ndot'][:,np.abs(out_exp['Mh'].value-Mi).argmin()],
        c=f'C{ii}', ls=lstyles[ii], label=None)
axs[1].plot(out_old['z'], out_old['tevol'],
    c=f'C0', label='Classical model')
axs[1].plot(out_exp['z'], out_exp['tevol'],
    c='C0', ls=lstyles[0], label='Exponent growth')
for ax in axs:
    ax.legend()
