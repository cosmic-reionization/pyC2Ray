...
class Source:
    ...
    def _heat_thick_integrand_vec(self, freq, tau):
        photo_thick = self._photo_thick_integrand_vec(freq, tau) # same as L675 in radiation_tables.f90 (C2Ray with helium)
...
class Source_Multifreq:
    ...
    def __init__(self, grey=None, freq0=None, pl_index=None, S_star_ref=None):
        ...
        self.R_star = 1.0
        self.S_star_ref = S_star_ref
        self.freqs_tab, self.crossect_HI, self.crossect_HeI, self.crossect_HeII = np.loadtxt(pc2r.__path__[0]+'/tables/Verner1996_spectidx.txt')
        
        self.NumBndin1 = np.count_nonzero(self.crossect_HI) - np.count_nonzero(self.crossect_HeI)
        self.NumBndin2 = np.count_nonzero(self.crossect_HeI) - np.count_nonzero(self.crossect_HeII)
    ...
    def _photo_thick_integrand_vec(self, freq, tau, pl_index):
        itg = self.SED(freq) * np.exp(-tau*self.cross_section_freq_dependence(freq, pl_index)) # same as L611 in radiation_tables.f90 (C2Ray with helium)
        
        # To avoid overflow in the exponential, check
        return np.where(tau*self.cross_section_freq_dependence(freq, pl_index) < 700.0, itg, 0.0)
    
    def _photo_thin_integrand_vec(self, freq, tau, pl_index):
        itg = self.SED(freq) * self.cross_section_freq_dependence(freq, pl_index) * np.exp(-tau*self.cross_section_freq_dependence(freq, pl_index))
        return np.where(tau*self.cross_section_freq_dependence(freq, pl_index) < 700.0, itg, 0.0)
    
    def _heat_thick_integrand_vec(self, freq, tau, pl_index, ion_freq):
        photo_thick = self._photo_thick_integrand_vec(freq, tau, pl_index) # same as L675 in radiation_tables.f90 (C2Ray with helium)
        ...
        for i in range(self.freqs_tab.size - 1):
            ...
            if((freq_min >= f_min) and (freq_max <= f_max)):
                ...
                if(self.crossect_HI[i] != 0):
                    integrand_thin = lambda f : self._photo_thin_integrand_vec(f, tau, self.crossect_HI[i])
                    integrand_thick = lambda f : self._photo_thick_integrand_vec(f, tau, self.crossect_HI[i])

                    table_thin_HI += quad_vec(integrand_thin, f_min, f_max, epsrel=1e-12)[0]
                    table_thick_HI += quad_vec(integrand_thick, f_min, f_max, epsrel=1e-12)[0]

                if(self.crossect_HeI[i] != 0):
                    integrand_thin = lambda f : self._photo_thin_integrand_vec(f, tau, self.crossect_HeI[i])
                    integrand_thick = lambda f : self._photo_thick_integrand_vec(f, tau, self.crossect_HeI[i])

                    table_thin_HeI += quad_vec(integrand_thin, f_min, f_max, epsrel=1e-12)[0]
                    table_thick_HeI += quad_vec(integrand_thick, f_min, f_max, epsrel=1e-12)[0]

                if(self.crossect_HeII[i] != 0):
                    integrand_thin = lambda f : self._photo_thin_integrand_vec(f, tau, self.crossect_HeII[i])
                    integrand_thick = lambda f : self._photo_thick_integrand_vec(f, tau, self.crossect_HeII[i])

                    table_thin_HeII += quad_vec(integrand_thin, f_min, f_max, epsrel=1e-12)[0]
                    table_thick_HeII += quad_vec(integrand_thick, f_min, f_max, epsrel=1e-12)[0]
            else:
                # if frequency min and max is outside the table frequency range
                pass

        return table_thin_HI, table_thick_HI, table_thin_HeI, table_thick_HeI, table_thin_HeII, table_thick_HeII
...
class BlackBodySource(Source):
    ...
    def SED(self,freq):
        if (((freq*u.Hz)*cst.h/cst.k_B/(self.temp*u.K)).cgs.value < 700.0):
            ...
            ampl = 8.0*np.pi**2 * (self.R_star*u.Rsun)**2 * cst.h / cst.c**2 * (freq*u.Hz)**5
            sed = amp / (np.exp((freq*u.Hz)*cst.h/cst.k_B/(self.temp*u.K))-1.0)
        else:
            sed = 0.0
        return sed
