...
class Run21cmfast:
    ...
    def __init__(self, HII_DIM, BOX_LEN, n_jobs=4, 
                 h=0.67, Om=0.31, Ob=0.045, s8=0.82, ns=0.96,
                 random_seed=42, 
                 data_dir='./21cmFAST_data',
                 INITIAL_REDSHIFT=300, CLUMPING_FACTOR=2.0,
                 **kwargs):
    ...
    def set_parameters(self):
        params = {
                "HII_DIM": self.HII_DIM, "DIM": self.HII_DIM*3,
    ...
    def simulate_matter_21cmfast(self, redshift_list):
        ...
        dens_dict = {}
        halo_catalog_dict = {}
        with p21c.global_params.use(INITIAL_REDSHIFT=self.INITIAL_REDSHIFT, CLUMPING_FACTOR=self.CLUMPING_FACTOR):
            for redshift in redshift_list:
                ...
                print('Assuming BOX_LEN is in Mpc. Halo catalogs catalogs have masses in Msol and positions in Mpc.')
                ...
                halo_list = {
                            ...
                            'z': redshift, 'BOX_LEN': Lbox,
                            }

                dens_dict[redshift] = dens
                halo_catalog_dict[redshift] = halo_list
                savedata = {
                            'density': dens, 'halo_list': halo_list, 
                            'cosmo_params': cosmo_params.__dict__, 
                            'user_params': user_params.__dict__,
                            }
                filename = self.data_dir+f'/matter_data_{Lbox:.2f}Mpc_{user_params.HII_DIM}_z{redshift:05.2f}.pkl'
